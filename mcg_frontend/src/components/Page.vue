            ...
            <div class="mcr-shell__header-grid">
              ...
              <div class="blueprint-hero-actions">
                ...
                <div class="yh-preview-chips" aria-label="当前参数">
                  <span class="yh-status-chip" :class="{ 'is-disabled': !pluginEnabled }">插件{{ pluginEnabled ? '已启用' : '已停用' }}</span>
    ...
    <Teleport to="body">
      <Transition name="yh-compact-header">
        ...
        >
          ...
          <v-btn class="mcr-button mcr-button--ghost mcr-button--dark-neutral yh-icon-btn yh-header-control" icon title="关闭" aria-label="关闭" @click="notifyClose">
...
async function measureCustomLayout(layout: CustomStaticLayout | null) {
  ...
  try {
    const resp = await props.api.post<{
      ...
      msg?: string
    }>('plugin/YahahaCoverStudio/measure_custom_static_layout', { layout })
...
async function saveRenderOptions() {
  ...
  try {
    ...
    const resp = await props.api.post<{
      ...
      msg?: string
    }>('plugin/YahahaCoverStudio/set_render_options', payload)
    if (resp && resp.code !== 0) {
...
async function saveAnimatedSettings() {
  ...
  try {
    ...
    const resp = await props.api.post<{
      ...
      data?: Partial<StatusPayload>
      msg?: string
    }>('plugin/YahahaCoverStudio/set_animated_settings', payload)
    if (resp && resp.code !== 0) {
...
async function executeHistoryRestore() {
  ...
  try {
    const resp = await props.api.post<{ code: number; data?: { restored?: number; skipped?: number; failed?: number }; msg?: string }>('plugin/YahahaCoverStudio/restore_history_batch', { batch_id: batchId })
...
async function persistCustomLayoutState() {
  ...
  try {
    const resp = await props.api.post<{
      ...
      msg?: string
    }>('plugin/YahahaCoverStudio/set_custom_static_layout', payload)
...
async function deleteSchemeItem(item: SchemeListItem) {
  ...
  try {
    const resp = await props.api.post<{
      ...
      msg?: string
    }>(`plugin/YahahaCoverStudio/delete_custom_static_template?id=${encodeURIComponent(id)}`)
...
async function loadStatusInner(): Promise<boolean> {
  ...
  try {
    const resp = await props.api.get<{ code: number; data?: StatusPayload; msg?: string }>('plugin/YahahaCoverStudio/status')
...
async function loadHistory() {
  ...
  try {
    const resp = await props.api.get<{ code: number; data?: HistoryItem[]; msg?: string }>('plugin/YahahaCoverStudio/history')
    ...
    if (resp?.code === 0) {
      if (resp.data?.images?.length) {
        ...
        try {
          ...
          // Cache persistence is optional; the refresh itself already succeeded.
          console.warn('preview cache persistence failed', cacheError)
        }
      }
      // A successful forced refresh can retain the existing artwork when no newer
      // candidate is available; do not report that successful request as a failure.
      return true
    }
    if (resp) {
...
async function startGeneration() {
  ...
  try {
    const targetStyle = coverStyleBase.value === 'custom_static'
      ? 'custom_static'
...
async function fetchCoverPayload(path: string) {
  const resp = await props.api.get<{
    ...
    msg?: string
  }>(`plugin/YahahaCoverStudio/download_saved_cover?file=${encodeURIComponent(path)}`)
...
async function downloadSelectedCoversZip() {
  ...
  try {
    const resp = await props.api.post<{
      ...
      data?: { name: string; mime: string; b64: string }
      msg?: string
    }>('plugin/YahahaCoverStudio/download_saved_covers', { files: paths })
    if (!resp || resp.code !== 0 || !resp.data?.b64) {
...
/* Brand typography and the compact command bar deliberately share the same
 * curated, subset-capable faces as the live cover preview. */
.mcr-page-shell .yh-brand-en-big {
