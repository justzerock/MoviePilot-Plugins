...
function renderTextLayer(layer: CustomTitleLayer | CustomTextLayer, source: PreviewSourcePayload | null, options: RenderOptions) {
  ...
  const family = getFontFamily(layer, text)
  const shadowNode = shadow
    ? `<text font-family="${family}" font-size="${fontSize}" font-weight="700" fill="${escapeXml(shadow.color)}" opacity="${shadow.opacity}" text-anchor="${textAnchor}" style="filter: blur(${shadow.blur}px);">${shadowTspans}</text>`
    : ''
