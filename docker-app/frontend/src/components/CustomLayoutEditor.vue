...
function refreshImageRegistry() {
  ...
  for (const layer of getEditableLayers(internalLayout.value.layers)) {
...
async function loadStickerLibrary() {
  ...
  try {
    const resp = await props.api.get<{ code: number; data?: StickerLibraryItem[]; msg?: string }>('plugin/MediaCoverGenerator/stickers')
    if (!resp || resp.code !== 0) {
...
async function deleteStickerItem(item: StickerLibraryItem) {
  ...
  try {
    const endpoint = `plugin/MediaCoverGenerator/delete_sticker?file=${encodeURIComponent(item.path)}`
...
async function uploadStickerPayload(payload: {
  ...
  try {
    ...
    }>('plugin/MediaCoverGenerator/upload_sticker', {
