...
export interface SettingsAnchorSection {
  id: string
  label: string
}
...
function setupObserver() {
  ...
  observer = new IntersectionObserver((entries) => {
    ...
    const visible = entries
      ...
      .sort((a, b) => Math.abs(a.boundingClientRect.top - props.topOffset) - Math.abs(b.boundingClientRect.top - props.topOffset))
    if (visible[0]?.target.id) activeId.value = visible[0].target.id
  }, {
    root: props.scrollContainer || null,
    rootMargin: `-${props.topOffset}px 0px -62% 0px`,
...
.yh-settings-anchor__line {
  width: 16px;
  height: 2px;
  border-radius: 999px;
  background: rgba(60, 60, 67, 0.25);
  transition: width 180ms ease, opacity 180ms ease, background-color 180ms ease, transform 180ms ease;
}

.yh-settings-anchor__button:hover .yh-settings-anchor__line,
.yh-settings-anchor__button:focus-visible .yh-settings-anchor__line {
  width: 24px;
  background: rgba(60, 60, 67, 0.55);
}

.yh-settings-anchor__button.is-active .yh-settings-anchor__line {
  width: 32px;
...
.yh-settings-anchor__button:focus-visible {
  outline: 2px solid color-mix(in srgb, var(--color-primary, #4f8cff) 42%, transparent);
  outline-offset: -3px;
  border-radius: 10px;
}

:global([data-mcr-theme="dark"]) .yh-settings-anchor__line {
  background: rgba(235, 235, 245, 0.22);
}

:global([data-mcr-theme="dark"]) .yh-settings-anchor__button:hover .yh-settings-anchor__line,
:global([data-mcr-theme="dark"]) .yh-settings-anchor__button:focus-visible .yh-settings-anchor__line {
  background: rgba(235, 235, 245, 0.5);
}

:global([data-mcr-theme="dark"]) .yh-settings-anchor__button.is-active .yh-settings-anchor__line {
  background: var(--color-primary, #6ea2ff);
}

@media (max-width: 1023px) {
  .yh-settings-anchor { display: none; }
}

@media (prefers-reduced-motion: reduce) {
  .yh-settings-anchor__line { transition: none; }
}
</style>
