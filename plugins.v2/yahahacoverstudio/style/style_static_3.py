...
def create_style_static_3(library_dir, title, font_path, font_size=(170,75), font_offset=(0,40,40), is_blur=False, blur_size=50, color_ratio=0.8, resolution_config=None, bg_color_config=None):
    ...
    输入:
      ...
      title_zh: 中文标题文本。
      title_en: 英文标题文本。
      main_title_font_path: 首选的中文字体文件路径 (可以是None)。
      subtitle_font_path: 首选的英文字体文件路径 (可以是None)。
    返回:
    ...
    try:
        ...
        if float(main_title_font_size) <= 0:
            main_title_font_size = 170
        if float(subtitle_font_size) <= 0:
        ...
        if resolution_config and resolution_config.height > 0:
            ...
            if scale_ratio > 0:
                main_title_font_size = main_title_font_size / scale_ratio
                subtitle_font_size = subtitle_font_size / scale_ratio
        ...
        text_shadow_color = darken_color(blur_color, 0.8)
        main_title_font_size = float(main_title_font_size) * scale
        result = draw_text_on_image(
            result, library_ch_name, (s(73.32), s(427.34) + main_title_font_size * main_title_font_offset), main_title_font_path, "ch.ttf", int(max(1, round(main_title_font_size))),
        ...
        if library_eng_name:
            ...
            # base_font_size = 50 * float(subtitle_font_size)  # 默认字体大小
            base_font_size = float(subtitle_font_size) * scale  # 默认字体大小
            ...
            main_title_font = ImageFont.truetype(main_title_font_path, int(max(1, round(main_title_font_size))))
            subtitle_font = ImageFont.truetype(subtitle_font_path, int(font_size))

            zh_bbox = draw.textbbox((0, 0), title_zh, font=main_title_font)
            zh_text_w = zh_bbox[2] - zh_bbox[0]

            en_bbox = draw.textbbox((0, 0), library_eng_name, font=subtitle_font)
