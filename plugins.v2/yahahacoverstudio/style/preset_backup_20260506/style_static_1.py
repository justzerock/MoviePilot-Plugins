...
def create_style_static_1(image_path, title, font_path, font_size=(170,75), font_offset=(0,40,40), blur_size=50, color_ratio=0.8, resolution_config=None, bg_color_config=None):
    try:
        ...
        main_title_font_size = float(font_size[0])
        subtitle_font_size = float(font_size[1])
        ...
        if not float(main_title_font_size) > 0:
            main_title_font_size = resolution_config.get_font_size(170)
        if not float(subtitle_font_size) > 0:
            subtitle_font_size = resolution_config.get_font_size(75)
        ...
        with managed_image(image_path, "RGB") as original_img:
            ...
            # 使用动态字体大小
            main_title_font = ImageFont.truetype(main_title_font_path, int(main_title_font_size))
            subtitle_font = ImageFont.truetype(subtitle_font_path, int(subtitle_font_size))
