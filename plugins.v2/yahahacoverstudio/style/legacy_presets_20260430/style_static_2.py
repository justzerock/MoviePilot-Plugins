...
def create_style_static_2(image_path, title, font_path, font_size=(170,75), font_offset=(0,40,40), blur_size=50, color_ratio=0.8, resolution_config=None, bg_color_config=None):
    try:
        main_title_font_path, subtitle_font_path = font_path
        title_zh, title_en = title
        main_title_font_size, subtitle_font_size = font_size
        main_title_font_offset, title_spacing, en_line_spacing = font_offset
        ...
        if not float(main_title_font_size) > 0:
            main_title_font_size = 170
        if not float(subtitle_font_size) > 0:
            subtitle_font_size = 75
        style_font_scale = 1.1
        main_title_font_size = float(main_title_font_size) * style_font_scale
        subtitle_font_size = float(subtitle_font_size) * style_font_scale
        ...
        # main_title_font_size = int(canvas_size[1] * 0.17 * float(zh_font_size_ratio))
        # subtitle_font_size = int(canvas_size[1] * 0.07 * float(en_font_size_ratio))
        
        main_title_font = ImageFont.truetype(main_title_font_path, main_title_font_size)
        ...
        # 计算中文标题的位置
        zh_bbox = draw.textbbox((0, 0), title_zh, font=main_title_font)
        zh_text_w = zh_bbox[2] - zh_bbox[0]
        zh_text_h = zh_bbox[3] - zh_bbox[1]
        
        # 定义英文行间距
        en_line_spacing = int(subtitle_font_size * 0.3)  # 英文行间距为字体大小的30%
        ...
        if title_en:
            # 检查英文标题是否需要分行
            en_bbox = draw.textbbox((0, 0), title_en, font=subtitle_font)
        ...
        zh_y = total_text_y + float(main_title_font_offset)

        # 中文标题阴影效果
        for offset in range(3, shadow_offset + 1, 2):
            current_shadow_color = shadow_color[:3] + (shadow_alpha,)
            shadow_draw.text((zh_x + offset, zh_y + offset), title_zh, font=main_title_font, fill=current_shadow_color)
        
        # 中文标题
        draw.text((zh_x, zh_y), title_zh, font=main_title_font, fill=text_color)
        ...
        if en_lines:
            ...
            for i, line in enumerate(en_lines):
                ...
                for offset in range(2, shadow_offset // 2 + 1):
                    current_shadow_color = shadow_color[:3] + (shadow_alpha,)
                    shadow_draw.text((en_x + offset, current_y + offset), line, font=subtitle_font, fill=current_shadow_color)
                
                # 英文标题
                draw.text((en_x, current_y), line, font=subtitle_font, fill=text_color)
