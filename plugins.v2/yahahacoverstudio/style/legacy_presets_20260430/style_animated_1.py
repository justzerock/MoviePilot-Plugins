...
def _build_text_layer(target_w, target_h, title, font_path, font_size, font_offset, bg_color, scale):
    ...
    main_title_font_offset, title_spacing, _ = font_offset
    main_title_font_path, subtitle_font_path = font_path
    title_zh, title_en = title

    # 参考 style_animated_1：按分辨率比例缩放字体
    main_title_font = ImageFont.truetype(main_title_font_path, max(1, int(main_title_font_size * scale)))
    subtitle_font = ImageFont.truetype(subtitle_font_path, max(1, int(subtitle_font_size * scale)))
    ...
    en_line_spacing = max(1, int(subtitle_font_size * 0.12 * scale))
    en_lines = []
    total_en_height = 0
    ...
    zh_y = total_text_y + float(main_title_font_offset) * scale
...
def create_style_animated_1(
    ...
    try:
        ...
        text_layer = _build_text_layer(
            ...
            title=title,
            font_path=font_path,
            font_size=(main_title_font_size, subtitle_font_size),
