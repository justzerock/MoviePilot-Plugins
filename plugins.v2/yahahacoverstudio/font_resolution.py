...
# Keep this runtime-neutral module behavior aligned with the Docker app.
...
def get_font_codepoints(font_path: str | Path, face_index: int = 0) -> frozenset[int]:
    path = Path(font_path)
    return _cached_font_codepoints(_font_cache_key(path, face_index)) if path.is_file() else frozenset()
...
def check_font_text_coverage(font_path: str | Path, text: str, face_index: int = 0) -> FontCoverage:
    ...
    return FontCoverage(bool(codepoints) and not missing, missing, tuple(ord(character) for character in missing), len(characters))
...
def _conversion_profiles(target: str, traditional_variant: str) -> tuple[str, ...]:
    ...
    if target == "simplified": return ("t2s",)
    if target == "traditional": return (traditional,)
...
def _convert(text: str, profile: str) -> str:
    try: return str(_opencc(profile).convert(text))
    except Exception: return text


def _script_name(profile: str | None) -> str:
    if profile == "t2s": return "simplified"
    if profile in {"s2t", "s2tw", "s2hk"}: return "traditional"
    return "original"


def resolve_render_text_and_font(original_text: Any, requested_font_path: str | Path, fallback_font_path: str | Path = "", *, adaptation_enabled: bool = True, target: str = "auto", traditional_variant: str = "standard", face_index: int = 0) -> ResolvedRenderText:
    ...
    if adaptation_enabled:
        ...
        for order, profile in enumerate(_conversion_profiles(str(target or "auto"), str(traditional_variant or "standard"))):
            ...
            if converted in seen: continue
            seen.add(converted)
            coverage = check_font_text_coverage(requested, converted, face_index)
            edits = sum(1 for left, right in zip(original, converted) if left != right) + abs(len(original) - len(converted))
            candidates.append((profile, converted, coverage, edits * 10 + order))
...
def font_internal_names(font_path: str | Path, face_index: int = 0) -> list[str]:
    ...
    try:
        ...
        for record in font["name"].names if "name" in font else []:
            if record.nameID not in {1, 2, 4, 6}: continue
